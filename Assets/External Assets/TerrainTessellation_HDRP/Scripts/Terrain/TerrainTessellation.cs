...
public class TerrainTessellation : MonoBehaviour
{
	...
	public void Initalize()
	{
		...
		for (int a = 0; a < disValue.Length; a++)
			disValue[a] = 1f;

		smoothnessValue = new float[splats.Length];
		for (int b = 0; b < smoothnessValue.Length; b++)
			smoothnessValue[b] = 0;

		normalPower = new float[splats.Length];
		for (int c = 0; c < normalPower.Length; c++)
			normalPower[c] = 1f;

		uvS = new float[splats.Length];
		for (int d = 0; d < uvS.Length; d++)
			uvS[d] = 10f;
